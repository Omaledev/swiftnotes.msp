...
export function initializeRealTimeFeatures(teamId, noteId = null) {
    ...
    teamChannel
        .listen('.note.created', (data) => {
            showNotification(`New note: ${data.note.title}`, 'success');
            if (shouldReloadNotesList()) setTimeout(refreshNotesList, 300);
        })
        .listen('.note.updated', (data) => {
            showNotification(`Note updated: ${data.note.title}`, 'info');
            isOnNotePage(data.note.id) ? updateNoteContent(data.note) :
            shouldReloadNotesList() && setTimeout(refreshNotesList, 300);
        })
        .listen('.user.status.updated', (data) => {
            showNotification(`${data.user.name} is now ${data.isOnline ? 'online' : 'offline'}`,
                           data.isOnline ? 'success' : 'warning');
            updateUserStatusIndicator(data.user.id, data.isOnline);
        });
    ...
    if (noteId) {
        ...
        noteChannel
            .listen('.user.editing', (data) => {
                if (data.user.id !== window.user?.id) addEditingIndicator(data.user);
            })
            .listen('.user.stopped.editing', (data) => {
                removeEditingIndicator(data.user.id);
            });
    ...
    window.addEventListener('load', () => {
        axios.post('/api/user-online', { is_online: true, team_id: teamId })
             .catch(console.error);
    });

    window.addEventListener('beforeunload', () => {
        axios.post('/api/user-online', { is_online: false, team_id: teamId })
             .catch(console.error);
    });
}
